...
async function init(options = {}) {
  ...
  if (kickoffEnabled) {
    ...
    console.log(chalk.gray('  This folder will hold your product docs — backlogs, PRDs, and project context.\n'));
    ...
    const answers = await inquirer.prompt([
      {
        type: 'input',
        name: 'workspaceName',
        message: 'Workspace folder name (this folder will hold your product docs — backlogs, PRDs, and project context):',
  ...
  const runFileCreation = () => {
    ...
    const contextDir = path.join(targetDir, projName, 'context');
    ...
    const projectContextVars = {
      ...
      constraints: kickoffData && kickoffData.product.constraints ? kickoffData.product.constraints : '{None identified yet}',
      commonNonGoals: '{To be filled — add product-level non-goals shared across all stories}',
    };
    const projectContextContent = renderTemplate(path.join(templatesDir, 'PROJECT_CONTEXT.md.tmpl'), projectContextVars);
    fs.writeFileSync(path.join(contextDir, 'PROJECT_CONTEXT.md'), projectContextContent, 'utf-8');
    createdFiles.push(`${wsName}/${projName}/context/PROJECT_CONTEXT.md`);
    ...
    const techStackBlock = kickoffData
      ...
      : '- Language:\n- Framework:\n- Database:\n- Testing:';
    const techContextContent = renderTemplate(path.join(templatesDir, 'TECH_CONTEXT.md.tmpl'), { projectName: projName, techStack: techStackBlock });
    fs.writeFileSync(path.join(contextDir, 'TECH_CONTEXT.md'), techContextContent, 'utf-8');
    createdFiles.push(`${wsName}/${projName}/context/TECH_CONTEXT.md`);

    const contextReadmeContent = renderTemplate(path.join(templatesDir, 'context-README.md.tmpl'), { projectName: projName });
    fs.writeFileSync(path.join(contextDir, 'README.md'), contextReadmeContent, 'utf-8');
    createdFiles.push(`${wsName}/${projName}/context/README.md`);
