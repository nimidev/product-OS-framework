...
/** Cursor MDC at project root; never overwrites if already present (US-005 T3). */
function writeCursorRuleIfMissing(projectRoot, templatesDir, projName, createdFiles) {
  const tmplPath = path.join(templatesDir, 'product-os.mdc.tmpl');
  if (!fs.existsSync(tmplPath)) return;
  const rel = path.join('.cursor', 'rules', 'product-os.mdc');
  const full = path.join(projectRoot, rel);
  if (fs.existsSync(full)) return;
  const content = renderTemplate(tmplPath, { projectName: projName });
  const dir = path.dirname(full);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(full, content, 'utf-8');
...
async function init(options = {}) {
  ...
  const runScaffold = () => {
    ...
    writeCursorRuleIfMissing(cwdAtStart, templatesDir, projName, createdFiles);
  ...
  console.log(
    chalk.gray(
      '  4. `.cursor/rules/` was added for Cursor Project Rules. Commit it so the team shares the same rules, or keep it local—your choice.\n'
    )
  );
}
